    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.password == password:
                print(f"Password match for user: {
                      user.email}")  # Debug information
                login_user(user)
...
@login_required
def dashboard():
    print(f"Loading dashboard for user: {current_user.email}, Role: {
          current_user.role}")  # Debug information
...
def create_ticket():
    if request.method == 'POST':
        ...
        print(f"Creating ticket with Title: {title}, Description: {
              description}, Priority: {priority}")  # Debug information
        new_ticket = Ticket(title=title, description=description,
                            priority=priority, status='open', user_id=current_user.id)
...
def ticket_details(ticket_id):
    print(f"Loading ticket details for ticket ID: {
          ticket_id}")  # Debug information
    ticket = Ticket.query.get_or_404(ticket_id)
    if request.method == 'POST':
        comment_text = request.form.get('comment_text')
        print(f"Adding comment to ticket ID: {ticket_id}, Comment: {
              comment_text}")  # Debug information
...
def unassigned_tickets():
    ...
    return render_template('unassigned_tickets.html', unassigned_tickets=unassigned_tickets, support_staff=support_staff)
...
def assign_ticket(ticket_id):
    ...
    return render_template('assign_ticket.html', ticket=ticket, support_staff=support_staff)
